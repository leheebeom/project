    ...
    xhr.onreadystatechange = () => {
        if (xhr.readyState === XMLHttpRequest.DONE) {
            ...
            if (xhr.status >= 200 && xhr.status < 300) {
                ...
                switch (responseJson['result']) {
                    case 'success' :
                        recoverEmailWarning.show('인증이 성공적으로 완료되었습니다.');
                    ...
                    case 'failure_expired' :
                        recoverEmailWarning.show('해당 인증 번호가 만료되었습니다.\n\n 처음부터 다시 시도해주세요.');
                        recoverEmailForm['contact'].removeAttribute('disabled');
                        recoverEmailForm['contactAuthRequestButton'].removeAttribute('disbaled');
                    ...
                    default :
                        alert('인증 실패');
                        recoverEmailWarning.show('인증번호가 일치하지 않습니다. 인증번호를 다시 입력해 주세요.');
                        break;
                }
            } else {
                recoverEmailWarning.show('서버와 통신하지 못하였습니다. 잠시 후 다시 시도해 주세요.');
...
recoverEmailForm.onsubmit = e => {
    ...
    formDate.append('contact', recoverEmailForm['contact'].value);
    formDate.append('code', recoverEmailForm['contactAuthCode'].value);
    formDate.append('salt', recoverEmailForm['contactAuthSalt'].value);

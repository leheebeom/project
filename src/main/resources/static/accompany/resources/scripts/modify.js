...
const setSelectedContinent = (value) => {
    ...
    const continentElement = continentContainer
...
form.onsubmit = e => {
    ...
    formDate.append('continentValue', getSelectedContinent()['value']);
    formDate.append('countryValue', getSelectedCountry()['value']);
    formDate.append('regionValue', getSelectedRegion()['value']);
    formDate.append('capacity', form['capacity'].value);
    formDate.append('dateFromStr', form['dateFrom'].value);
    formDate.append('dateToStr', form['dateTo'].value);
    formDate.append('title', form['title'].value);
    formDate.append('content', editor.getData());
    if ((form['coverImage']?.files?.length ?? 0) > 0) {
        formDate.append('coverImageFile', form['coverImage'].files[0]);
    }
...
let editor;
const loadArticle = () => {
    // const id = parse(window.location.href.split('/').at(-1).split('?')[0]);
